...
export class DuplicateDetectionEnhancements2026073000004 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`ALTER TABLE persons ADD COLUMN IF NOT EXISTS merged_into_id UUID REFERENCES persons(id)`);
  }

  async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`DROP INDEX IF EXISTS idx_persons_name_trgm`);
    await queryRunner.query(`ALTER TABLE persons DROP COLUMN IF EXISTS merged_into_id`);
  }
}
