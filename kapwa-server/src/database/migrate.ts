...
async function migrate() {
  ...
  await q.query(`CREATE TABLE IF NOT EXISTS family_members ( id UUID PRIMARY KEY DEFAULT uuid_generate_v7(), household_id UUID REFERENCES households(id), full_name TEXT NOT NULL, relationship TEXT NOT NULL, age INTEGER, status_income TEXT, is_primary BOOLEAN DEFAULT FALSE )`);
  ...
  await q.query(`CREATE TABLE IF NOT EXISTS interventions ( id UUID PRIMARY KEY DEFAULT uuid_generate_v7(), case_id UUID REFERENCES cases(id), intervention_type TEXT, amount DECIMAL(12,2), fund_source TEXT CHECK (fund_source IN ('Regular','PDAF','Legislative','Donation')), agency TEXT, service_date DATE NOT NULL, voucher_no TEXT, or_reference TEXT, worker_signature_url TEXT NOT NULL, logged_by UUID, logged_at TIMESTAMP DEFAULT NOW(), hash TEXT, prev_hash TEXT )`);
  await q.query(`CREATE TABLE IF NOT EXISTS case_tracker_log ( id UUID PRIMARY KEY DEFAULT uuid_generate_v7(), daily_seq_num INTEGER NOT NULL, transaction_date DATE NOT NULL, surname TEXT, first_name TEXT, middle_name TEXT, gender TEXT CHECK (gender IN ('M','F')), age_range TEXT CHECK (age_range IN ('0-7','8-17','18-59','60+')), client_category TEXT, barangay TEXT, intervention_remarks TEXT, created_at TIMESTAMP DEFAULT NOW(), UNIQUE (transaction_date, daily_seq_num) )`);
