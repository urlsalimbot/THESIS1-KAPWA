...
async function migrate() {
  ...
  await q.query(`
    CREATE TABLE IF NOT EXISTS households (
      id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
      primary_beneficiary_id UUID REFERENCES beneficiaries(id),
      barangay TEXT,
  ...
  await q.query(`
    CREATE TABLE IF NOT EXISTS cases (
      ...
      status TEXT CHECK (status IN ('pending_assessment','in_review','approved','disbursed','closed')) DEFAULT 'pending_assessment',
  ...
  await q.query(`
    CREATE TABLE IF NOT EXISTS interventions (
      ...
      voucher_no TEXT,
      or_reference TEXT,
      worker_signature_url TEXT NOT NULL,
      logged_by UUID,
  ...
  await q.query(`
    CREATE TABLE IF NOT EXISTS access_card_services (
      id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  ...
  await q.query(`
    CREATE TABLE IF NOT EXISTS irf_cases (
      id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  ...
  await q.query(`
    CREATE POLICY ben_admin_all ON beneficiaries
  ...
  await q.query(`
    CREATE POLICY ben_barangay_scope ON beneficiaries
      FOR ALL USING (
        ...
        AND (current_setting('app.current_barangay') = '' OR address ILIKE '%' || current_setting('app.current_barangay') || '%')
      );
  `);
  await q.query(`
  ...
  await q.query(`
    CREATE POLICY cases_barangay_scope ON cases
      FOR ALL USING (
        current_setting('app.current_role') IN ('social_worker', 'coordinator')
        AND EXISTS (
          SELECT 1 FROM beneficiaries b WHERE b.id = cases.beneficiary_id
          AND (current_setting('app.current_barangay') = '' OR b.address ILIKE '%' || current_setting('app.current_barangay') || '%')
        )
      );
  `);
  await q.query(`
    CREATE POLICY int_admin_all ON interventions
      FOR ALL USING (current_setting('app.current_role') = 'admin');
  `);
  await q.query(`
    CREATE POLICY consent_admin_all ON consent_ledger
      FOR ALL USING (current_setting('app.current_role') = 'admin');
  `);
  await q.query(`
    CREATE POLICY consent_self ON consent_ledger
      FOR SELECT USING (
        current_setting('app.current_role') = 'social_worker'
