...
async function migrate() {
  ...
  await q.query(`CREATE TABLE IF NOT EXISTS interventions ( id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), case_id UUID REFERENCES cases(id), intervention_type TEXT CHECK (intervention_type IN ('FA','C','CSR','R','H','HV','Other')), amount DECIMAL(12,2), fund_source TEXT CHECK (fund_source IN ('Regular','PDAF','Legislative','Donation')), agency TEXT, service_date DATE NOT NULL, voucher_no TEXT, or_reference TEXT, worker_signature_url TEXT NOT NULL, logged_by UUID, logged_at TIMESTAMP DEFAULT NOW(), hash TEXT )`);
