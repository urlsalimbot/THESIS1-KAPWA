...
**93/93 tests passing**, TS clean (server + client). **23 of 50 bugs fixed** (all CRITICAL + HIGH + key MEDIUM).
...
| 19 | `kapwa-server/src/beneficiaries/beneficiaries.service.ts:50` | Barangay search uses exact match (`address: barangay`) but addresses are full text | MEDIUM | ❌ |
| 20 | `kapwa-server/src/csr/csr.service.ts:16-17` | Control number race condition — concurrent requests get same count → UNIQUE violation | MEDIUM | ❌ |
| 21 | `kapwa-server/src/csr/csr.controller.ts:46` | `POST` used for PDF download — should be `GET` | MEDIUM | ✅ FIXED |
| 22 | `kapwa-server/src/audit/audit.service.ts:9-24` | All methods are stubs returning empty/fake data | MEDIUM | ❌ |
...
| 26 | `kapwa-client/src/components/Layout.tsx:18,20,24` | Sidebar links to `/filing`, `/irf`, `/audit`, `/settings` — none exist → redirect to `/` | MEDIUM | ❌ (removed dead nav items) |
...
| # | File | Bug | Severity | Status |
|---|------|-----|----------|--------|
| 30 | `kapwa-server/src/app.module.ts:32` | `SnakeNamingStrategy` defined but never wired to TypeORM | LOW | ❌ |
| 31 | `kapwa-server/src/main.ts:11-14` | Global `ValidationPipe` dead code (codebase uses Zod) | LOW | ❌ |
| 32 | `kapwa-server/src/chat/chat.gateway.ts:39` | `userId` can be `undefined` if JWT lacks `sub`/`id` | LOW | ❌ |
| 33 | `kapwa-server/src/chat/chat.gateway.ts:63-82` | No rate limiting on WebSocket messages | LOW | ❌ |
| 34 | `kapwa-server/src/cases/cases.controller.ts:40` | `@Get('disbursed/pending-intervention')` after `@Get(':id')` — fragile ordering | LOW | ❌ |
| 35 | `kapwa-server/src/database/seed.ts:18` | `TRUNCATE` missing `sync_queue`, `chat_messages`, `csr_reports` etc. | LOW | ❌ |
...
| 38 | `kapwa-client/src/pages/IntakePage.tsx:184` | `resetForm` clears state but `<SignaturePad>` canvas still shows old drawing | LOW | ❌ |
| 39 | `kapwa-client/src/pages/DashboardPage.tsx:112` | "View All Cases" button has no `onClick` | LOW | ✅ FIXED |
| 40 | `kapwa-client/src/pages/BeneficiariesPage.tsx:61` | Only 5 of 10 barangays in dropdown | LOW | ❌ |
| 41 | `kapwa-client/src/pages/InterventionsPage.tsx:39` | Hardcoded signature URL `'sig-placeholder.png'` | LOW | ❌ |
| 42 | `kapwa-client/src/pages/CaseTrackerPage.tsx:99` | `computeAgeRange()` defined but never called | LOW | ❌ |
| 43 | `kapwa-client/src/lib/sync.ts:17` | `payload` variable computed but never used | LOW | ❌ |
| 44 | `kapwa-client/src/lib/database.ts:42` | `execute()` is a silent no-op returning `[]` | LOW | ❌ |
| 45 | `kapwa-client/src/lib/sync.ts:109-111` | Caches to `kapwa_cache_*` keys but never reads — localStorage bloat | LOW | ❌ |
| 46 | `kapwa-client/src/App.tsx:3-8` | `AppProvider` and `useConfig` exported but never imported | LOW | ❌ |
| 47 | `kapwa-client/src/lib/auth.ts` | Functions `async` but only do sync `localStorage` | LOW | ❌ |
| 48 | `kapwa-client/src/components/forms/SignaturePad.tsx:14-15` | `hasContent` and `isEmpty` track same condition (always inverses) | LOW | ❌ |
| 49 | `kapwa-client/src/pages/MessagesPage.tsx:33-36` | No dedup on socket reconnection — duplicate messages | LOW | ❌ |
