...
export function MayorWidgets() {
  ...
  const stats: { label: string; value: string; icon: typeof CheckCircle; color: string }[] = [
    ...
    { label: t('dashboard.uniqueHouseholds', 'Unique Households'), value: String(dataOrEmpty.uniqueHouseholds || 0), icon: Users, color: 'bg-emerald-100 text-emerald-800' },
    ...
    { label: t('dashboard.slaCompliance', 'SLA Compliance'), icon: CheckCircle, color: 'bg-emerald-100 text-emerald-800', value: t('dashboard.compliant', 'Compliant') },
  ];

  const slaIcon = dataOrEmpty.slaCompliance?.slaStatus === 'compliant' ? CheckCircle : AlertTriangle;
  const slaColor = dataOrEmpty.slaCompliance?.slaStatus === 'compliant' ? 'bg-emerald-100 text-emerald-800' : 'bg-destructive/10 text-destructive';
