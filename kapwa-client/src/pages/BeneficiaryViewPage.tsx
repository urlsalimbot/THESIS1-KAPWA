...
export function BeneficiaryViewPage() {
  ...
  const { data: cases } = useSWR<Array<Record<string, unknown>>>(
  ...
  const { data: trackerEntries, isLoading: trackerLoading } = useSWR<
    TrackerEntry[]
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2 text-sm text-muted-foreground">
                  <span className="flex items-center gap-1">
                    <User size={13} /> {beneficiary.gender}, {beneficiary.age}{" "}
                    yrs
                  </span>
                  <span className="flex items-center gap-1">
                      ...
                      state: {
                        prefill: {
                          ...
                          estimatedMonthlyIncome:
                            (
                              ben.estimatedMonthlyIncome as number
                            )?.toString() || "",
                          philhealthNumber:
                            (ben.philhealthNumber as string) || "",
                        },
                      },
                    });
                  }}
              ...
              {beneficiary.cases.length === 0 ? (
                ...
                <div className="space-y-1.5 max-h-60 overflow-y-auto">
                  {beneficiary.cases.map((c) => (
                    <div
                      key={c.id}
                      className="flex items-center justify-between rounded bg-muted/50 px-2.5 py-2 text-sm"
                    >
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        ...
                        {c.status === "disbursed" && (
                          <Button
                            ...
                            onClick={() =>
                              setInterventionCaseId(
                                c.id === interventionCaseId ? null : c.id,
                              )
                            }
                          >
              ...
              {beneficiary.interventions.length === 0 ? (
                ...
                <div className="space-y-1.5 max-h-60 overflow-y-auto">
                  {beneficiary.interventions.map((intv) => (
                    <div
                      key={intv.id}
                      className="rounded bg-muted/50 px-2.5 py-2 text-sm"
                    >
                ...
                <div className="space-y-1.5 max-h-60 overflow-y-auto">
                  {(trackerEntries || []).map((entry) => (
                    ...
                    >
                      <div className="min-w-0 flex-1 mr-2">
                        <p className="font-medium text-foreground truncate">
                          {entry.trackerId || "Entry"}
                        </p>
                        <p className="text-xs text-muted-foreground truncate">
                          {entry.interventionRemarks}
                        </p>
                      </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div className="space-y-1">
                    ...
                    <select
                      ...
                      value={intForm.type}
                      onChange={(e) =>
                        setIntForm({ ...intForm, type: e.target.value })
                      }
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setIntForm({ ...intForm, amount: e.target.value })
                      }
                      aria-label="Amount"
                    />
                  </div>
                  <div className="space-y-1">
                    ...
                    <select
                      className="flex h-9 w-full rounded-md border border-input bg-background px-2 py-1 text-sm"
                      value={intForm.fundSource}
                      onChange={(e) =>
                        setIntForm({ ...intForm, fundSource: e.target.value })
                      }
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      type="file"
                      accept="image/*"
                      className="flex h-9 w-full rounded-md border border-input bg-background px-2 py-1 text-sm file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-foreground"
                      onChange={(e) =>
                        setIntReceiptFile(e.target.files?.[0] || null)
                      }
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-lg bg-card p-4 shadow-sm border border-border">
            ...
            <div className="space-y-2">
              <InfoRow
                icon={Calendar}
                label="Birth Date"
                value={beneficiary.birthDate || "N/A"}
              />
              <InfoRow
                icon={Phone}
                label="Contact"
                value={beneficiary.contact || "N/A"}
              />
              <InfoRow
                icon={Tag}
                label="Category"
                value={beneficiary.category || "N/A"}
              />
              <InfoRow
                icon={Home}
                label="Household"
                value={`${beneficiary.householdSize} members`}
              />
            </div>
          </div>
          ...
          {family.length > 0 && (
            <div className="rounded-lg bg-card shadow-sm border border-border">
              <button
                ...
                className="flex items-center gap-2 text-primary w-full px-4 py-3 text-left"
              >
                {familyExpanded ? (
                  <ChevronDown size={14} />
                ) : (
                ...
                <div className="px-4 pb-3 space-y-1.5 max-h-64 overflow-y-auto">
                  {family.map((m, i) => {
                    ...
                    return (
                      <div
                        key={m.id}
                        className={`rounded-lg ${bgColors[i % 3]} px-3 py-2 transition-colors hover:bg-muted/50`}
                      >
                        <div className="flex items-center justify-between gap-2">
                          <div className="flex items-center gap-2.5 min-w-0 flex-1">
                            <div
                              className={`relative flex h-7 w-7 shrink-0 items-center justify-center rounded-full ${m.isPrimary ? "bg-primary text-primary-foreground" : "bg-card text-muted-foreground"} text-[10px] font-semibold shadow-sm`}
                            >
                              {m.fullName.charAt(0)}
                              <span
                                className={`absolute -bottom-0.5 -right-0.5 h-2 w-2 rounded-full ${dotColors[i % 3]} ring-1 ring-card`}
                              />
                            </div>
                            <div className="min-w-0">
                              <div className="flex items-center gap-1.5">
                                ...
                                {m.isPrimary && (
                                  <span className="rounded bg-primary/20 px-1 py-0.5 text-[9px] font-medium text-primary leading-none">
                                    Primary
                                  </span>
                                )}
                              </div>
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            {m.status && (
                              <span className="rounded-full bg-muted/60 px-1.5 py-0.5 text-[9px] font-medium text-muted-foreground leading-none">
                                {m.status}
                              </span>
                            )}
                            {m.income != null && (
          ...
          <div className="rounded-lg bg-card p-4 shadow-sm border border-border">
            ...
            <FamilyGraph
              ...
              error={
                famError
                  ? (famError as any)?.message || "Failed to load family graph"
                  : null
              }
