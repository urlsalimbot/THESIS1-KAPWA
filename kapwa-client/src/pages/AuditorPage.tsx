...
export function AuditorPage() {
  ...
  const consentColumns: ColumnDef<any>[] = [
    ...
    { accessorKey: 'purpose', header: t('dashboard.purpose', 'Purpose') },
    {
      ...
      cell: ({ row }) => (
        <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${row.original.status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-destructive/10 text-destructive'}`}>
          {row.original.status}
        </span>
      ),
    },
    ...
    >
      ...
      {activeTab === 'hash' && (
        <div>
            ...
            {[
              {
                ...
                color: 'bg-emerald-50 text-emerald-700', icon: Shield,
              },
              {
                label: t('audit.integrity', 'Integrity'),
                value: allValid ? t('dashboard.compliant', 'Intact') : t('audit.tampered', 'Tampered'),
                color: allValid ? 'bg-emerald-50 text-emerald-700' : 'bg-destructive/10 text-destructive', icon: allValid ? CheckCircle : XCircle,
              },
              {
          ...
          <div className={`rounded-lg p-4 mb-4 flex items-center gap-3 ${allValid ? 'bg-emerald-50 border border-emerald-200' : 'bg-destructive/10 border border-destructive/20'}`}>
            {allValid ? <CheckCircle className="text-emerald-600" size={24} /> : <XCircle className="text-destructive" size={24} />}
            <div>
              <p className={`font-semibold ${allValid ? 'text-emerald-800' : 'text-destructive'}`}>
                ...
                {Object.entries(hashChain).map(([table, status]: [string, any]) => (
                  <div key={table} className="flex items-center justify-between px-4 py-3">
                    <div className="flex items-center gap-2">
                      {status.valid ? <CheckCircle size={16} className="text-emerald-600" /> : <XCircle size={16} className="text-destructive" />}
