    ...
    <div className="relative flex items-center justify-center min-h-screen px-4 bg-background">
      <Card className="w-full max-w-md mx-auto shadow-lg border-border/50">
        <CardHeader className="text-center pb-6">
          <CardTitle className="text-2xl tracking-tight">{t('auth.emailVerification', 'Email Verification')}</CardTitle>
        </CardHeader>
        <CardContent className="text-center space-y-4">
          {status === 'verifying' && (
            <div className="flex flex-col items-center gap-3 py-4">
              <Loader2 size={40} className="animate-spin text-muted-foreground" />
              <p className="text-muted-foreground">{t('auth.verifyingEmail', 'Verifying your email...')}</p>
            </div>
          )}
          {status === 'success' && (
            <div className="flex flex-col items-center gap-3 py-4">
